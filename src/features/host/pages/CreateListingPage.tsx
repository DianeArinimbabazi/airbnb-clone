...
export function CreateListingPage() {
  ...
  return (
    <div style={{ maxWidth:"680px", margin:"0 auto", padding:"40px 24px 80px" }}>
      ...
      <h1 style={{ fontSize:"26px", fontWeight:800, color:text, margin:"0 0 8px" }}>Create a new listing</h1>
      <p style={{ color:sub, fontSize:"14px", marginBottom:"32px" }}>Fill in the details about your property</p>

      <form onSubmit={handleSubmit(onSubmit)} style={{ display:"flex", flexDirection:"column", gap:"24px" }}>

        {/* Photos */}
        <div>
          <label style={lbl}>Property Photos <span style={{ fontWeight:400, color:sub }}>(up to 10, max 5MB each)</span></label>
          <div
            style={{ border:`2px dashed ${border}`, borderRadius:"12px", padding:"24px", textAlign:"center", background:card, cursor:"pointer" }}
            onClick={() => document.getElementById("img-upload")?.click()}
          >
            {previews.length === 0 ? (
              <div>
                <p style={{ fontSize:"32px", margin:"0 0 8px" }}></p>
                <p style={{ color:sub, fontSize:"14px", margin:"0 0 4px" }}>Click to upload photos</p>
                <p style={{ color:sub, fontSize:"12px", margin:0 }}>JPEG, PNG, WebP up to 5MB each</p>
              </div>
            ) : (
              <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill, minmax(120px, 1fr))", gap:"8px" }}>
                ...
                {previews.length < 10 && (
                  <div style={{ height:"100px", border:`2px dashed ${border}`, borderRadius:"8px", display:"flex", alignItems:"center", justifyContent:"center", fontSize:"24px", color:sub }}>+</div>
                )}
              </div>
            )}
          </div>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:"12px" }}>
          ...
          <label htmlFor="superhost" style={{ fontWeight:600, fontSize:"14px", color:text, cursor:"pointer" }}>
        ...
        {/* Progress */}
        {uploadProgress && (
          <div style={{ background: dark ? "#064e3b" : "#f0fdf4", border:`1px solid ${dark ? "#065f46" : "#d1fae5"}`, borderRadius:"10px", padding:"14px", display:"flex", alignItems:"center", gap:"10px" }}>
            ...
            <p style={{ margin:0, fontSize:"14px", color: dark ? "#6ee7b7" : "#059669", fontWeight:500 }}>{uploadProgress}</p>
          </div>
        )}
